...
assert.match(reportModeSource, /复制 Markdown/, 'report mode should allow copying the markdown report')
assert.match(reportModeSource, /复制简报 JSON/, 'report mode should allow copying the research brief json')
assert.match(reportModeSource, /navigator\.clipboard\.writeText/, 'report mode should use the Clipboard API for copy actions')
assert.match(reportModeSource, /JSON\.stringify\(brief\.value,\s*null,\s*2\)/, 'report mode should format the copied brief json')
assert.match(reportModeSource, /copyStatus/, 'report mode should show copy feedback')
